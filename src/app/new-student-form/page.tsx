...
import {
  ...
  levelOptions,
  newStudentApplicationSchema,
  religionOptions,
  secondaryStreamOptions,
  type NewStudentApplicationFormData
} from '@/lib/applicationConstants';
import { applicationAPI } from '@/services/api';
import { zodResolver } from '@hookform/resolvers/zod';
import { BookOpen, Briefcase, Calendar, FileText, GraduationCap, IdCard, Mail, MapPin, Phone, User, Users } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useEffect, useRef, useState } from 'react';
import { useForm } from 'react-hook-form';

/**
 * New Student Application Form Page
 * 
 * Allows new students (freshmen) to submit housing applications.
 */
export default function NewStudentFormPage() {
  ...
  const {
    ...
    reset,
    watch,
  } = useForm<NewStudentApplicationFormData>({
    resolver: zodResolver(newStudentApplicationSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: NewStudentApplicationFormData) => {
    try {
      ...
      await applicationAPI.submitApplication({
        studentType: 0, // 0 for new student
        studentInfo: {
        ...
        secondaryInfo: {
          secondaryStream: data.secondaryInfo.secondaryStream,
          totalScore: Number(data.secondaryInfo.totalScore),
          percentage: Number(data.secondaryInfo.percentage),
          grade: data.secondaryInfo.grade,
        },
        academicInfo: {
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold text-[#132a4f]">طلب إسكان - طالب مستجد</h1>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Secondary Education Information Section */}
            <div className="border-b border-gray-200 pb-6">
              <h2 className="text-xl font-bold text-[#132a4f] mb-4 flex items-center gap-2">
                <GraduationCap size={20} className="text-[#0292B3]" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormSelect
                  ...
                  options={secondaryStreamOptions}
                  error={errors.secondaryInfo?.secondaryStream?.message}
                  required
                  {...register('secondaryInfo.secondaryStream')}
                />
                <FormInput
                  label="المجموع الكلي"
                  placeholder="مثال: 380"
                  type="number"
                  error={errors.secondaryInfo?.totalScore?.message}
                  required
                  {...register('secondaryInfo.totalScore')}
                />
                <FormInput
                  ...
                  type="number"
                  step="0.01"
                  error={errors.secondaryInfo?.percentage?.message}
                  required
                  {...register('secondaryInfo.percentage')}
                />
                <FormSelect
                  label="التقدير"
                  placeholder="اختر التقدير"
                  options={gradeOptions}
                  error={errors.secondaryInfo?.grade?.message}
                  required
                  {...register('secondaryInfo.grade')}
                />
              </div>
            </div>
            ...
            <div className="border-b border-gray-200 pb-6">
              <h2 className="text-xl font-bold text-[#132a4f] mb-4 flex items-center gap-2">
                <BookOpen size={20} className="text-[#0292B3]" />
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button
                ...
                className="flex-1 bg-linear-to-r from-[#0d3a52] to-[#0d5a7a] hover:from-[#0d5a7a] hover:to-[#0d7a9a] text-white font-semibold py-3 rounded-lg transition-all duration-200 disabled:opacity-50"
              >
